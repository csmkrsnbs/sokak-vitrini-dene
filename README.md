...
- OpenAI Images Edit API ile iki referans fotoğraf üzerinden gerçek önizleme
...
- OpenAI Image Edit API (`gpt-image-2` varsayılan)
...
- OpenAI API anahtarı ve görsel model erişimi
...
1. Projeyi GitHub deposuna gönderin ve Vercel'de **New Project** ile içe aktarın.
2. Neon panelinde **Connect** düğmesinden **pooled** bağlantı adresini alın.
3. Vercel projesinde **Settings → Environment Variables** bölümüne aşağıdaki
   değerleri ekleyin:
   - `DATABASE_URL`
   - `OPENAI_API_KEY`
   - `OPENAI_IMAGE_MODEL=gpt-image-2`
   ...
   - `RATE_LIMIT_SALT`
   - `CRON_SECRET`
4. Deploy işlemini başlatın. `vercel-build` komutu migration'ı uygular ve
   üretim derlemesini oluşturur.
5. Yayın bittikten sonra `https://alan-adiniz.com/api/health` adresini açın.
...
lib/server/openai-image.ts   OpenAI görsel düzenleme entegrasyonu
...
`DATABASE_URL` ile `OPENAI_API_KEY` bulunması gerekir.
